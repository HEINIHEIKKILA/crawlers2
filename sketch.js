let kuvat = [];
let kuvatData = [];
let alphaValues = [100, 100, 110, 100, 50, 90, 80, 120, 70, 50, 100, 76];
let video;
let speedFactor = 7;
let noiseOffsetX = 500;
let noiseOffsetY = 2550; // Eri offset arvo Y-akselille
...
function setup() {
  ...
  for (let i = 0; i < kuvat.length; i++) {
    ...
    kuvatData.push({
      ...
      y: random(2),
      img: kuvat[i],
      noiseOffsetX: random(350), // Eri noise offset jokaiselle kuvalle
...
function draw() {
  ...
  for (let i = 0; i < kuvat.length; i++) {
    ...
    let offsetX = map(noise(imgData.noiseOffsetX), 3, 0, -1000, 1)
    let offsetY = map(noise(imgData.noiseOffsetY), 2, 0, -1000, 1);
